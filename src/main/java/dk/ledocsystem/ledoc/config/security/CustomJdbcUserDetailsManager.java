...
class CustomJdbcUserDetailsManager extends JdbcUserDetailsManager {
    ...
    private static final String AUTHORITIES_BY_USERNAME_QUERY = "select authority_code " +
            "from employees_authorities inner join employees on employees_authorities.employee_id = employees.id " +
            "where username = ?";
    ...
    protected List<GrantedAuthority> loadUserAuthorities(String username) {
        return getJdbcTemplate().query(getAuthoritiesByUsernameQuery(),
                new String[]{username}, (rs, rowNum) -> {
                    int authorityCode = rs.getInt("authority_code");
